let faq = document.getElementsByClassName("faq-box-question");
let icons = document.getElementsByClassName("faq-icon");

for (const singleFaq of faq) {
  singleFaq.addEventListener("click", function () {
    this.classList.toggle("active");

    let icon = this.querySelector("path");

    let answer = this.nextElementSibling;
    if (this.classList.contains("active")) {
      icon.setAttribute("fill", "#301534");
      icon.setAttribute(
        "d",
